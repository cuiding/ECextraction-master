...
class ACNN(object):
    ...
    def __init__(
            ...
            embedding_size, filter_sizes, filter_emotion_sizes, num_filters, hidden_dim, num_layers, drop_keep_gru,
            learning_rate, window_size, num_features, l2_reg_lambda=0.0004):
        ...
        def pad_for_wide_conv(x):
            return tf.pad(x, np.array([[0, 0], [0, 0], [window_size - 1, window_size - 1], [0, 0]]), "CONSTANT",
                          name="pad_wide_conv")
        ...
        def attention_machanism(x1, x2):
            ...
            post_matrix = tf.reshape(tf.reduce_sum(tf.tensordot(tf.transpose(x1, [1, 0, 2, 3]), post_att, 0), -2),
                                     x2.get_shape())
